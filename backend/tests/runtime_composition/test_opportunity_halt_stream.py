...
class CapturedAckBlockingFakeStream(BlockingFakeStream):
    def run(self) -> None:
        self.run_calls += 1
        self.supervisor.connection_starting()
        self.supervisor.subscription_message(
...
def _config(*, codebook_hash: str | None = None, symbol: str = "NVDA") -> HaltAuthorityConfig:
...
def _adapter(*, codebook: HaltStatusCodebook | None = None, epochs=("epoch-1", "epoch-2")):
    ...
    def recovery_timer_factory(interval, function):
        timer = FakeRecoveryTimer(interval, function)
        stream.recovery_timers.append(timer)
        return timer
    ...
    adapter = AlpacaOpportunityHaltStreamAdapter(
        ...
        recovery_timer_factory=recovery_timer_factory,
    )
...
@pytest.mark.parametrize("clock_lead", [timedelta(seconds=4), timedelta(seconds=31)])
def test_boundary_read_uses_adapter_clock_when_stream_is_ahead(clock_lead: timedelta) -> None:
    adapter, stream, clock, _ = _adapter()
    boundary = clock.value
    _ack(adapter, stream, clock)
    clock.value = boundary + clock_lead
    asyncio.run(stream.trade_handler(_trade(clock.value)))

    snapshot = OpportunityHaltRuntimeAdapter(adapter, symbol="NVDA").read(
        symbol="NVDA",
        trusted_at=boundary,
    )

    assert snapshot.state is HaltAuthorityState.OPEN_CONFIRMED
    assert snapshot.observed_at == clock.value
    assert snapshot.last_trade_at == clock.value


def test_unknown_live_stream_resubscribes_after_backoff_and_recovers(caplog) -> None:
    caplog.set_level(logging.INFO)
    adapter, stream, clock, threads = _adapter(epochs=("epoch-1", "epoch-2"))
    _ack(adapter, stream, clock)
    adapter.start()
    clock.value += timedelta(seconds=1)
    asyncio.run(stream.trade_handler(_trade(clock.value)))
    stream.supervisor.stream_error()

    assert stream.recovery_timers[-1].interval == 1.0
    assert stream.recovery_timers[-1].daemon is True
    stream.recovery_timers[-1].fire()
    failed = adapter.snapshot(read_at=clock.value)

    assert failed.state is HaltAuthorityState.UNKNOWN
    assert threads[0].is_alive()
    assert stream.trade_subscribe_calls == 2
    assert failed.epoch == "epoch-1"
    ...
    clock.value += timedelta(seconds=1)
    asyncio.run(stream.trade_handler(_trade(clock.value, 2)))

    recovered = adapter.snapshot(read_at=clock.value)
    assert recovered.state is HaltAuthorityState.OPEN_CONFIRMED
    assert recovered.epoch == "epoch-2"
    assert "trigger=ERROR state=OPEN_CONFIRMED->UNKNOWN" in caplog.text
    assert "halt authority resubscribe requested" in caplog.text
    assert "trigger=EPOCH_ACK state=UNKNOWN->UNKNOWN" in caplog.text
    assert "trigger=TRADE state=UNKNOWN->OPEN_CONFIRMED" in caplog.text


def test_stale_live_stream_resubscribes_without_clearing_fail_closed_state() -> None:
    adapter, stream, clock, _ = _adapter(epochs=("epoch-1", "epoch-2"))
    _ack(adapter, stream, clock)
    adapter.start()
    clock.value += timedelta(seconds=1)
    asyncio.run(stream.trade_handler(_trade(clock.value)))
    ...
    stale = adapter.snapshot(read_at=clock.value)

    assert stale.state is HaltAuthorityState.UNKNOWN
    assert stale.acknowledged_at is not None
    assert stream.trade_subscribe_calls == 1

    stream.recovery_timers[-1].fire()
    recovering = adapter.snapshot(read_at=clock.value)

    assert recovering.state is HaltAuthorityState.UNKNOWN
    assert recovering.acknowledged_at is None
    assert stream.trade_subscribe_calls == 2


def test_recovery_ack_does_not_clear_a_real_halt_latch() -> None:
    codebook = _codebook()
    adapter, stream, clock, _ = _adapter(
        codebook=codebook,
    ...
    recovered = adapter.snapshot(read_at=clock.value)
    assert recovered.state is HaltAuthorityState.HALTED_LATCHED
    assert recovered.trading_halt_state is TradingHaltState.HALTED
...
def test_start_accepts_exact_transport_ack_before_session_without_halt_authority() -> None:
    stream = CapturedAckBlockingFakeStream()
    clock = Clock(OPEN - timedelta(minutes=30))
    adapter = AlpacaOpportunityHaltStreamAdapter(
        stream,
        config=_config(),
        codebook=None,
        clock=clock,
        epoch_factory=lambda: "epoch-pre-session",
        join_timeout=1,
    )

    adapter.start()
    transport_snapshot = adapter.snapshot(read_at=clock.value)
    clock.value += timedelta(seconds=1)
    asyncio.run(stream.trade_handler(_trade(clock.value)))
    pre_session_snapshot = adapter.snapshot(read_at=clock.value)
    adapter.stop()

    assert transport_snapshot.state is HaltAuthorityState.UNKNOWN
    assert transport_snapshot.trading_halt_state is TradingHaltState.UNKNOWN
    assert transport_snapshot.acknowledged_at is None
    assert pre_session_snapshot.state is HaltAuthorityState.UNKNOWN
    assert pre_session_snapshot.trading_halt_state is TradingHaltState.UNKNOWN
...
def test_pinned_sdk_data_timeout_reconnects_a_connected_but_mute_socket() -> None:
    stream = PinnedAlpacaStockDataStream("key", "secret", data_timeout=0.01)
    recorder = SupervisorRecorder()
    stream.bind_supervisor(recorder)
    ...
    assert asyncio.run(stream._consume()) is True
    assert socket.closed is True
    assert recorder.events == [("closed", None)]
